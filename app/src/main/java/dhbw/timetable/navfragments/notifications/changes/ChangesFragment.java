...
public class ChangesFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        final TextView notCritView = view.findViewById(R.id.NotCritText);
        final TextView notCritValueView = view.findViewById(R.id.NotCritValue);

        final TextView formView = view.findViewById(R.id.FormText);
        final TextView formValueView = view.findViewById(R.id.FormValue);

        final TextView toneView = view.findViewById(R.id.ToneText);
        final TextView toneValueView = view.findViewById(R.id.ToneValue);

        View.OnClickListener onCritClick = v -> ListDialog.newInstance("Select a criterion", (dialog, which) -> {
            if (which >= 0) {
        ...
        View.OnClickListener onFormClick = v -> ListDialog.newInstance("Select a form", (dialog, which) -> {
            if (which >= 0) {
                ListView lw = ((AlertDialog) dialog).getListView();
                String checkedItem = (String) lw.getAdapter().getItem(lw.getCheckedItemPosition());
                SharedPreferences.Editor editor = sharedPref.edit();
        ...
        View.OnClickListener onToneClick = v -> ListDialog.newInstance("Select a tone", (dialog, which) -> {
            if (which >= 0) {
                ListView lw = ((AlertDialog) dialog).getListView();
                String checkedItem = (String) lw.getAdapter().getItem(lw.getCheckedItemPosition());
                // Play the tone
